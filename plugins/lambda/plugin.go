...
func defaultConfig(manager *plugins.Manager) Config {
	// Take the cli flag inputs. If not specified, they default to the config defaults above.
	minimumTriggerThreshold := manager.MinimumTriggerThreshold
	triggerTimeout := manager.TriggerTimeout
	pluginStartPriority := defaultPluginStartPriority
	pluginStopPriority := defaultPluginStopPriority
...
func (p *PluginFactory) New(manager *plugins.Manager, config interface{}) plugins.Plugin {
	parsedConfig := defaultConfig(manager)
	if config != nil {
